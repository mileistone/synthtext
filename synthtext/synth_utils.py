...
class LUTOfRGB(object):
    """
    RGB LUT for Mayavi glyphs.
    """
    def __create_8bit_rgb_lut__():
        xl = np.mgrid[0:256, 0:256, 0:256]
        lut = np.vstack((xl[0].reshape(1, 256**3), xl[1].reshape(1, 256**3),
                         xl[2].reshape(1, 256**3), 255 * np.ones(
                             (1, 256**3)))).T
    ...
    def set_rgb_lut(myv_glyph):
        ...
        lut._vtk_obj.SetTableRange(0, LUTOfRGB.__lut__.shape[0])
        lut.number_of_colors = LUTOfRGB.__lut__.shape[0]
        lut.table = LUTOfRGB.__lut__
...
def plot_xyzrgb(xyz, rgb, show=False):
    ...
    """
    rgb_s = LUTOfRGB.rgb2scalar(rgb)
    pts_glyph = mym.points3d(xyz[:, 0],
                             xyz[:, 1],
                             xyz[:, 2],
                             rgb_s,
                             mode='point')
    LUTOfRGB.set_rgb_lut(pts_glyph)
    if show:
        mym.view(180, 180)
...
def implicit_plot(expr,
    ...
    cont1 = mym.pipeline.iso_surface(src,
                                     color=col_isurf,
                                     contours=[0],
                                     transparent=False,
                                     opacity=0.8)
...
def isplanar(xyz, sample_neighbors, dist_thresh, num_inliers, z_proj):
    ...
    dv = -np.percentile(xyz, 50,
                        axis=0)  # align the normal to face towards camera
    max_iter = sample_neighbors.shape[-1]
    plane_info = fit_plane_ransac(xyz,
